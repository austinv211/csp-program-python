...
class CSP(Generic[V, D]):
    ...
    def __init__(self, variables: List[V], assignment_limit=0) -> None:
    ...
    def complete_the_assignment(self, assignment: Dict[V, D], domains: Dict[V, Set[D]]) -> Optional[Dict[V, D]]:
        """
        ...
        for selected_value in sorted(domains[selected_variable]):
            ...
            self.assignments += 1
            reduced_domains = {v: (domains[v] - {selected_value}) for v in domains}
            ...
            result = self.complete_the_assignment(extended_assignment, reduced_domains)
